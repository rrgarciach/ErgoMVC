...
//    $('#popup-mensaje').popup();
    ...
    $('#input-id_producto').click(function() {
        $('#input-id_producto').val('');
    });
    $('#input-id_producto').focus();
...
//        return false;
//    });
    $('#input-id_producto').keypress(function(e) {
        if (e.which == 13) {
            var ean = $('#input-id_producto').val();
            $('#input-id_producto').val('');
            $.post(base_uri + '_controlador/renglones_controlador.php',
                    {f: 'capturarRenglonPorEAN', ean: ean},
            function(data) {
                if (data != 'vacio') {
//                    alert('XML!');
                    var xml = data;
...
//                    alert(data);
                    var snd = new Media("/android_asset/www/button-9.wav");
//                    var snd = new Media("button-9.wav");
//                    var snd = new Media(base_uri + "html/button-9.wav");
                    snd.play();
                    $('#input-id_producto').focus();
                    // hacer nada.
                }
...
//            document.getElementById('list').innerHTML = '<ul>' + output.join('') + '</ul>';
            ...
            reader.onload = (function(theFile) {
                return function(e) {
                    ...
                    var xml = $(xmlDoc);
                    $('#cliente').val(xml.find('consignacion').find('cliente').text());
                    var renglonNo = 0;
